...
# -*- coding: utf-8 -*-
"""
Unit tests for ``Scripts/generate-semibold-source.py`` (SemiBold Font Weight).

Covers:
    * CLI argument contract (zero / one / three arguments -> non-zero exit)
    * Input == output path guard (REQ-09)
    * Metadata mapping for upright and italic inputs (Spec v1.1 section 4.2)
    * ``changeWeight(<stroke>, "LCG", 0, 0, "retain")`` invocation (REQ-04, GUD-01)
    * ``removeOverlap`` / ``simplify`` invocation and runtime order (REQ-06)
    * Advance-width enforcement to 1060 on every glyph (REQ-05)
    * ``save`` target is the output path, never the input path (REQ-09)
...
SCRIPT_PATH = os.path.join(REPO_ROOT, "Scripts", "generate-semibold-source.py")


def _load_script():
    """Load ``Scripts/generate-semibold-source.py`` as a fresh module instance."""
    spec = importlib.util.spec_from_file_location(
        "generate_semibold_source", SCRIPT_PATH
    )
...
generate_semibold_source = _load_script()
...
class FakeGlyph:
    ...
    the reverted cleanup approach (Medium plan Dead-End #11).
    """
...
def _run_main(argv, script_module=generate_semibold_source):
...
class TestUprightGeneration:
    """REQ-07 + Spec v1.1 section 4.2 metadata for the SemiBold upright variant."""
    ...
    def test_metadata_and_names_for_upright_input(self, fake_fontforge):
        ...
        assert (
            _run_main(
                [
                    ...
                    "Sources/FantasqueSansMono-SemiBold.sfdir",
                ],
                script,
            )
            == 0
        ...
        font = fake.fonts[0]
        assert font.os2_weight == 600
        assert font.familyname == "Fantasque Sans Mono"
        assert font.fontname == "FantasqueSansMono-SemiBold"
        assert font.fullname == "Fantasque Sans Mono SemiBold"
        assert ("English (US)", "Family", "Fantasque Sans Mono") in font.sfnt_names
        assert ("English (US)", "SubFamily", "SemiBold") in font.sfnt_names
        assert (
            "English (US)",
            "Fullname",
            "Fantasque Sans Mono SemiBold",
        ) in font.sfnt_names
        assert (
            "English (US)",
            "PostScriptName",
            "FantasqueSansMono-SemiBold",
        ) in font.sfnt_names
    ...
    def test_weight_name_set_for_upright_input(self, fake_fontforge):
        ...
        assert fake.fonts[0].weight == "SemiBold"
...
class TestItalicDetection:
    ...
    def test_metadata_and_names_for_italic_input(self, fake_fontforge):
        ...
        assert (
            _run_main(
                [
                    ...
                    "Sources/FantasqueSansMono-SemiBoldItalic.sfdir",
                ],
                script,
            )
            == 0
        ...
        font = fake.fonts[0]
        assert font.os2_weight == 600
        assert font.fontname == "FantasqueSansMono-SemiBoldItalic"
        assert font.fullname == "Fantasque Sans Mono SemiBold Italic"
        assert ("English (US)", "SubFamily", "SemiBold Italic") in font.sfnt_names
        assert (
            "English (US)",
            "PostScriptName",
            "FantasqueSansMono-SemiBoldItalic",
        ) in font.sfnt_names
...
class TestGeometryPipeline:
    """REQ-04, REQ-05, REQ-06 operation sequence."""
    ...
    def test_embolden_and_cleanup_calls(self, fake_fontforge):
        ...
        font = fake.fonts[0]
        assert font._change_weight_calls == [
            (script.STROKE_WIDTH, "LCG", 0, 0, "retain")
        ]
    ...
    def test_geometry_operations_run_in_plan_order(self, fake_fontforge):
        """REQ-06: selection must precede embolden, then font-level cleanup."""
        fake, script = fake_fontforge
        _run_main(["in.sfdir", "out.sfdir"], script)
...
class TestSaveTarget:
    """REQ-09: never write to the input path."""
