...
- **Modifications to the zero-touch set (CON-07):** `Makefile`, `config.schema.json`, `configure.py`, `custom-build.yml`, `custom_build_driver.py`, `build.py`, `fontbuilder.py`, and `features.py` must remain untouched.
...
- **FR-01: SemiBold Source Generation Script** (Priority: P0 — Critical)
  ...
  - The script SHALL apply a single-pass embolden transformation via FontForge `changeWeight(stroke, "LCG", 0, 0, "retain")`. The reference stroke is approximately +55–70 em-units; the exact value SHALL be established empirically through specimen calibration during Phase 1 before sources are committed.
...
- **FR-02: SemiBold Italic Source Generation** (Priority: P0 — Critical)
  - The same script (or a clearly documented invocation) SHALL accept `FantasqueSansMono-Italic.sfdir` as input and produce `FantasqueSansMono-SemiBoldItalic.sfdir`.
...
- **FR-04: Automatic Build Pipeline Integration** (Priority: P0 — Critical)
  ...
  - SemiBold SHALL be compiled into TTF, OTF, WOFF, and WOFF2 output files in the appropriate `Variants/` subdirectories.
...
- **Counter Space Legibility (top risk):** Dense glyphs (`e`, `a`, `s`, `@`, `%`, `&`, `8`, `#`) must maintain legible inner counter spaces after emboldening. The SemiBold stroke is approximately double the Medium stroke, so the clogging risk is significantly higher than Medium. Residual artifacts follow the Medium accepted-deviation precedent, deferred to visual QA (GH-006).
...
- **Adoption Rate:** ≥ 10% of font downloads include the SemiBold weight within 3 months of the first release containing it.
- **Community Feedback:** No more than 2 valid glyph quality issues reported within 30 days of release.
- **Editor Compatibility:** SemiBold renders correctly and is selectable by name in the top 5 code editors (VS Code, JetBrains IDEs, Sublime Text, Neovim with GUI, Terminal.app / Windows Terminal).
...
- **Release Bundle Completeness:** 100% of release archives include SemiBold and SemiBold Italic variants.
...
- **Build Pipeline Zero-Touch:** The SemiBold weight compiles successfully in CI without any modifications to the CON-07 zero-touch set (`Makefile`, `config.schema.json`, `configure.py`, `custom-build.yml`, `custom_build_driver.py`, `build.py`, `fontbuilder.py`, `features.py`).
...
- **Stroke Reference Unknown (primary assumption):** Medium used +34 em-units (range +30..+40). No calibrated reference exists for weight 600. The reference stroke (~55–70 em-units) MUST be established empirically via specimen renders at candidate values (e.g., 50/60/70 em-units) compared against the Medium and Bold neighbors, before sources are committed.
- **Counter Space Clogging Scales with Stroke (top risk):** The Medium stroke already left residual self-intersections (252 upright / 465 italic, accepted via maintainer exception). A roughly doubled stroke significantly raises the clogging risk on dense glyphs (`e`, `a`, `s`, `@`, `%`, `&`, `8`, `#`). Manual glyph fixes may become necessary; the Spec must define an "ask first" boundary.
...
- **Phase 1 — Stroke Calibration & Source Generation (2 days):** Render specimen pages at candidate stroke values (~50/60/70 em-units) and compare against Medium and Bold neighbors; select the reference stroke. Create `generate-semibold-source.py`. Generate `FantasqueSansMono-SemiBold.sfdir` and `FantasqueSansMono-SemiBoldItalic.sfdir`. Run automated validation (advance width check, overlap removal, `validate-font`). Perform visual inspection of core ASCII glyphs.
...
- **Acceptance criteria**:
  - [ ] Running `make` with SemiBold `.sfdir` sources present in `Sources/` produces TTF, OTF, WOFF, and WOFF2 output files for SemiBold and SemiBold Italic in the appropriate `Variants/` subdirectories.
  ...
  - [ ] No modifications to the CON-07 zero-touch set (`Makefile`, `config.schema.json`, `configure.py`, `custom-build.yml`, `custom_build_driver.py`, `build.py`, `fontbuilder.py`, `features.py`) are required.
...
- **Acceptance criteria**:
  ...
  - [ ] WOFF2 and WOFF files for SemiBold are referenced in the CSS `src` descriptor.
...
- **Acceptance criteria**:
  ...
  - [ ] Dense glyphs (`e`, `a`, `s`, `@`, `%`, `&`, `8`, `#`) maintain discernible inner counter spaces. Residual self-intersections follow the `accepted-deviation` precedent (GH-001 AC3).
  - [ ] Specimen renders at the candidate stroke values are visually compared against the Medium and Bold neighbors before the reference stroke is selected.
...
- **Acceptance criteria**:
  ...
  - [ ] WOFF and WOFF2 variants for SemiBold and SemiBold Italic are included in the web font archive.
