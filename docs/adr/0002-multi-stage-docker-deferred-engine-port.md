...
- **Stage 2** (`Ubuntu 26.04 LTS` with **Python 3.14** installed explicitly via the deadsnakes PPA or pyenv, since the distro default may be 3.13) hosts only the `configure.py` wrapper (Python 3.14) and the post-build packaging tooling.
...
- Stage 2 provides a forward-looking Python 3.14 surface for the configuration layer (supports BG-5, the "forward-looking" goal).
